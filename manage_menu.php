...
if (!$manage_class->ValidateSession(true)) {
	...
	$tpl_links .= "</strong>";
	$tpl_links .= '<li><span id="postingpassword"><a id="showpwd" href="#" onclick="javascript:document.getElementById(\'postingpassword\').innerHTML = \'<input type=text id=postingpasswordbox value=' . $manage_postpassword . '>\'; document.getElementById(\'postingpasswordbox\').select(); return false;">'._gettext('Show Posting Password').'</a></span></li></ul>';
	$tpl_links .= section_html(_gettext('Home'), 'home');
	$tpl_links .= '<ul><li><a href="' . KU_WEBFOLDER . '" target="_top">' . _gettext('Home') . '</a></li>' . "\n";
	$tpl_links .= '<li><a href="manage_page.php?">'._gettext('View Announcements').'</a></li>';
	...
	$tpl_links .= '</ul></div>';
	// Home
	$tpl_links .= section_html(_gettext('Statistics'), 'stats') .
	'<ul>
	<li><a href="manage_page.php?action=stats&view=list">' . _gettext('Statistics') . '</a>
	<li><a href="manage_page.php?action=statistics">' . _gettext('Graphs') . '</a>
	<li><a href="manage_page.php?action=posting_rates">'._gettext('Posting rates (past hour)').'</a></li>
	<li><a href="manage_page.php?action=stats&view=dead">' . _gettext('Low Activity') . '</a></li>';
	$tpl_links .= '</ul></div>';
	// Administration
	if ($manage_class->CurrentUserIsAdministrator()) {
		...
		$tpl_links .= '</ul></div>' .
		section_html(_gettext('Tokens'), 'tokens') .
		'<ul>
		<li><a href="manage_page.php?action=tokens&act=add">' . _gettext('Add tokens') . '</a></li>
		<li><a href="manage_page.php?action=tokens&act=view">' . _gettext('Edit tokens') . '</a></li>
		<li><a href="manage_page.php?action=addmodule&do=upload">' . _gettext('Upload Tokens') . 
'</a></li></ul></div>';
