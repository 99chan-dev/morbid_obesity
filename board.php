...
KU_DBPREFIX . "module` ORDER BY `id` DESC");
		if (count($results) > 0) {
			foreach($results as $line) {
			$modboards = explode("|",$line['boards']);
				foreach($modboards as $modboard) {
					if($_POST['board'] == $modboard) {
						if($line['trigger'] == $_POST['em']) {
							$post_message = $post_message . '<br/>' . $line['html'];
							$alreadydone = 'true';
						}
					}
				}
				if(($line['global'] == 1) && (!isset($alreadydone)))  {
					if($line['trigger'] == $_POST['em']) {
						$post_message = $post_message . '<br/>' . $line['html'];
					}
				}
	...
	foreach ($_POST['post'] as $val) {
		...
		if (isset($_POST['reportpost'])) {
			...
			if ($board_class->board['enablereporting'] == 1) {
				...
				if ($_POST['reportreason'] == null) { 
					exit("A reason for this report is required");
				}
