...
<div style="margin: 3em;">
	...
	{foreach name=bans item=ban from=$bans}
		...
		{/if} 
		<strong>{if $ban.globalban eq 1}{t}All boards{/t}{else}/{implode('/</strong>, <strong>/', explode('|', $ban.boards))}/{/if}</strong> {t}for{/t} {if $ban.url}{t}<a href="{/t}{$ban.url}{t}">this post</a> with {/t}{/if}{t}the following reason{/t}:<br /><br />
		...
		{if $.foreach.bans.last}
			<br />{t}Your IP address is{/t} <strong>{$.server.REMOTE_ADDR}</strong>. To be honest you shouldn't really worry about being banned, this is 99chan everyone gets banned here (even the admins) just wait for it to expire or get on our irc join #appeals and get ahold of someone in there. <3<br /><br />
		{/if}
