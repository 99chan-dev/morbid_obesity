...
$sqlquery = "SELECT ".KU_DBPREFIX."posts.id, parentid, file, file_type, message, thumb_w, thumb_h, ".KU_DBPREFIX."boards.name as name FROM 
...
$topads .= '<img src="http://wat.99chan.org/banners/top.php"><br>NSFW - ' . $result['name'] . '';
} else
$topads .= '<img src="'. KU_WEBPATH . '/' . $result['name'] . '/thumb/'. $result['file'] . 's.'. $result['file_type'] . '" width="'. 
$result['thumb_w'] . '" height="'. $result['thumb_h'] . '" border="0" />';
$topads .= '</a> <br> ' . $result['message'] . ''."\n".'</td>'."\n";
}
$topads .= "</tr></table></div>";
// - END MOD LAST PICS
